...
#  podman build  --network=host -f ubu24-iqm.dockerfile -t balewski/ubu24-iqm:p3b     --platform linux/arm64   
...
# Install Qiskit 1.x, IQM-Client, and related packages within the virtual environment
RUN echo "2b-AAAAAAAAAAAAAAAAAAAAAAAAAAAAA Qiskit  libs" && \
    /opt/venv/bin/pip install "qiskit<1.3" "qiskit[visualization]" qiskit-ibm-runtime qiskit-aer "iqm-client[qiskit]==34.0.1"

# Install additional Python libraries
RUN echo "2d-AAAAAAAAAAAAAAAAAAAAAAAAAAAAA python libs" && \
    /opt/venv/bin/pip install --upgrade pip && \
    /opt/venv/bin/pip install matplotlib h5py scipy scikit-learn jupyter notebook bitstring lmfit pytest networkx rustworkx pylatexenc
